...
- 취급 상품·작품·캐릭터·가격대·분위기·혼잡도·방문 팁 등 **매장 특성**만 3~5문장으로 요약하세요.
- rdb 쪽 "address" 필드는 CSV 기준 정제용이므로 knowledge_base_text와 별개입니다.
...
async def refine_shop(shop: ShopRecord, crawl_text: str) -> dict:
    ...
    반환값:
    {
        "is_figure_relevant": True | False,
        "rdb": { ... } | None,            # 가챠·피규어샵에 해당 안 하면 None → DB 저장 안 함
        "knowledge_base_text": str | None,  # 요약(주소 제외); 비대상이면 None
        "error": None | "오류 메시지"
    }
    """
    ...
    try:
        logger.info(
            "[refine] 단계=gemini_request | shop=%s | model=%s | crawl_자수=%s",
            shop.name,
        ...
        # 디버깅: 응답 출력
        if not text:
            logger.error("[refine] Gemini 빈 응답 (shop=%s)", shop.name)
            return {"rdb": None, "knowledge_base_text": None, "error": "Gemini 빈 응답"}

        logger.debug("[refine] Gemini 원응답_자수=%s shop=%s", len(text), shop.name)
